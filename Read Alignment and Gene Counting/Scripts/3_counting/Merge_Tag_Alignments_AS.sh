...
output_merge=$output_path'MergedBAM/DemplxBAM/'$time'/'$samplename'_merged_'$g'.bam'

java -jar $scriptpath'picard-2.7.1/picard.jar' MergeBamAlignment \
...
tag='GE'
output_tag=$output_path'MergedBAM/DemplxBAM/'$time'/'$samplename'_exontagged_'$g'.bam'

java -jar $scriptpath'Drop-seq_tools-1.12/jar/dropseq.jar' TagReadWithGeneExon \
	 I=$output_merge \
	 O=$output_tag \
...
output_file=$output$samplename'_'$g
...
java -jar $scriptpath'Drop-seq_tools-1.12/jar/dropseq.jar' DigitalExpression \
	 I=$input \
	 O=$output_temp \
