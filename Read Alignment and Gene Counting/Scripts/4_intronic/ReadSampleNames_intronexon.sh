...
while [ $count -lt $ncells ]; do
    ...
    then
	$script_path'Merge_Tag_Alignments_intronexon.sh' $reference $unaligned_file $aligned_file $output $includeMM $paired $samplename $unique_only $annotation $input_path $rowbarcode $output_path $time $script_path
	...
	$script_path'Merge_Tag_Alignments_AS_intronexon.sh' $reference $unaligned_file $aligned_file1 $output $includeMM $paired $samplename $unique_only $annotation $input_path $rowbarcode $output_path $time $g1 $script_path
	...
	$script_path'Merge_Tag_Alignments_AS_intronexon.sh' $reference $unaligned_file $aligned_file2 $output $includeMM $paired $samplename $unique_only $annotation $input_path $rowbarcode $output_path $time $g2 $script_path
