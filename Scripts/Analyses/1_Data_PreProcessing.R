...
for(allele in c("", "_B6", "_Cast")){
  for(w in c("", "_Spliced", "_Unspliced")){
    ...
    save(dge, file = paste0(dgepath, "UF_DGE", allele, w, ".RData"))
  }
}
...
load(paste0(dgepath, "UF_DGE.RData"))
...
load(paste0(dgepath, "UF_DGE_B6.RData")); b6 <- dge
load(paste0(dgepath, "UF_DGE_Cast.RData")); cast <- dge
...
for(f in list.files(path = dgepath, full.names = TRUE)){
  load(f)
...
load(paste0(dgepath, "CF_DGE.RData")); notas <- dge
detrate_notas <- rowMeans(notas$counts>0)

print("3.2.2) AS dropout rate")
load(paste0(dgepath, "CF_DGE_B6.RData")); b6 <- dge
load(paste0(dgepath, "CF_DGE_Cast.RData")); cast <- dge
...
files_DGE <- list.files(path = dgepath, full.names = TRUE)[grepl(x = list.files(path = dgepath, full.names = TRUE), pattern = "CF_") & !grepl(x = list.files(path = dgepath, full.names = TRUE), pattern = "pliced")]
...
library(scran)
load(paste0(dgepath, "GFCF_DGE.RData"))
...
save(sf, file = paste0(dgepath, "PoolClust_sizefactors_autosomal.RData"))

print("4.2) Store size factors in DGE lists")
files_DGE <- list.files(path = dgepath, full.names = TRUE)[grepl(x = list.files(path = dgepath, full.names = TRUE), pattern = "GFCF_")]
for(f in files_DGE){
...
load(paste0(dgepath, "NGFCF_DGE.RData")); xist_umi <- dge$counts["Xist",]
...
load(paste0(dgepath, "NGFCF_DGE_B6.RData")); b6 <- dge
load(paste0(dgepath, "NGFCF_DGE_Cast.RData")); cast <- dge
...
files_DGE <- list.files(path = dgepath, full.names = TRUE)[grepl(x = list.files(path = dgepath, full.names = TRUE), pattern = "NGFCF_")]
for(f in files_DGE){
