...
path <- "C:/Users/pacini/Desktop/git_review/Pacini_paper/analysis/"
...
<<>>=
library(ggplot2); library(biomaRt); library(GEOquery); library(data.table); library(plyr); library(dplyr); library(scran); library(edgeR); library(monocle); library(grid); library(velocyto.R); library(matrixStats); library(pcaMethods); library(umap); library(reshape2); library(MAST); library(pheatmap); library(multidplyr); library(parallel); library(plyr); library(dplyr); library(lsr); library(doParallel); library(tidyr); library(ggrepel); library(ggalluvial); library(readxl); library(data.table); library(UpSetR); library(openxlsx)
@
...
\section{Figure 3: Identification of putative Xist regulators}
<<>>=
start <- Sys.time()
source(paste0(scriptpath, "fig3_XistRegulators.R"))
end <- Sys.time()
time <- end - start; time
@

\section{Figure 4: Identification of putative X-chromosome regulators}
<<>>=
start <- Sys.time()
source(paste0(scriptpath, "fig4_XchrChangeRegulators.R"))
end <- Sys.time()
time <- end - start; time
@
...
\section{Figure 4: Chromosome-wide silencing dynamics}
<<>>=
start <- Sys.time()
source(paste0(scriptpath, "fig4_SilencingDynamics.R"))
end <- Sys.time()
time <- end - start; time
@

\section{Figure 5: Allelic silencing dynamics}
<<>>=
start <- Sys.time()
source(paste0(scriptpath, "fig5_AllelicSilencingDynamics.R"))
end <- Sys.time()
time <- end - start; time
@
...
<<>>=
start <- Sys.time()
source(paste0(scriptpath, "fig6_ExperimentalValidation.R"))
end <- Sys.time()
time <- end - start; time
@
