...
path <- "C:/Users/pacini/Desktop/git_final/Pacini_paper/analysis/"
...
\section{Figure 6: Chromosome-wide silencing dynamics}
<<>>=
start <- Sys.time()
...
\section{Figure 7: Allelic silencing dynamics}
<<>>=
start <- Sys.time()
...
\section{Figure 8: Experimental validation of differentially silenced genes}
<<>>=
start <- Sys.time()
