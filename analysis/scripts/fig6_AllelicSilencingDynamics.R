...
outpath <- paste0(path, "output/fig6_GeneAlleleSpecificSilencing/"); dir.create(path = outpath, recursive = T, showWarnings = F)
...
sigthreshold <- 0.05
binresults <- diff_silencing(x = df_sk)
...
print("7.3) Plot")
class_melt$k_ordered_classes <- plyr::revalue(factor(class_melt$k_ordered), replace = c("1" = "Fast",
                                                                                        "2" = "Intermediate",
                                                                                        "3" = "Slow",
                                                                                        "4" = "Escape"))
ngenes <- class_melt %>% 
...
silclasses <- c("Fast", "Intermediate", "Slow", "Escape")
