...
g <- ma[!ma$day %in% "0",] %>%  
  ...
  geom_text(data = test, aes(x = factor(day), y = 2.25, label = formatC(pvalue_wmw, digits = 1)), 
...
print("5) F: Gene-wise allele specific XP50 values")
...
adjust_size(g = g, panel_width_cm = 5, panel_height_cm = 5, savefile = paste0(outpath, "F_SilencingHalfTime.pdf"))
...
g <- gg_sub %>%  
  ...
  labs(x = "", 
       ...
       color = "")
adjust_size(g = g, panel_width_cm = 5, panel_height_cm = 3, savefile = paste0(outpath, "E_Comparison_Marks&Borensztein.pdf"))
...
g <- gcm_lisa %>%  
  ...
  geom_jitter(color = "black",
              alpha = 1/4, position=position_jitterdodge(jitter.width = .1, dodge.width = 0.75), 
              size = small_scattersize, show.legend = FALSE) +
  stat_summary(fun=median, aes(ymin=..y.., ymax=..y..), geom='errorbar', width=0.25,
               position=position_jitterdodge(jitter.width = 0, dodge.width = 0.75), size = linesize*2) +
  ...
  labs(x = "", 
       y = expression("Silencing percentage: (1-("*X[i.chrX]*"/("*X[i.chrX]*"+"*X[a.chrX]*"))*100"),
       color = "")
adjust_size(g = g, panel_width_cm = 4, panel_height_cm = 3, savefile = paste0(outpath, "E_Comparison_Sousa.pdf"))


print("7) G: Significantly differentially silenced genes")
...
adjust_size(g = g, panel_width_cm = 1.5, panel_height_cm = 2, savefile = paste0(outpath, "G_MostDEgenes.pdf"), height = 10)


print("8) G: Concordance in allelic silencing dynamics gene-wise classification")

print("8.1) Load data - K-means trend")
...
adjust_size(g = g, panel_width_cm = 1.5, panel_height_cm = 1.5, savefile = paste0(outpath, "H1_kmeansTrend.pdf"), height = 10)

print("8.3) Load data - Sankey")
...
adjust_size(g = g, panel_width_cm = 3, panel_height_cm = 5, savefile = paste0(outpath, "H2_SankeyPlot.pdf"), 
            height = 10, width = 10)
