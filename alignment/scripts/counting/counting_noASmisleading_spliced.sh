...
while [ $count -lt $ntime ]; do
    ...
    $script_path'counting/counting_noASmisleading_spliced_launch.sh' $script_path $input_path $time $reference $unaligned $aligned $includeMM $paired $unique_only $annotation $rowbarcode $sample_names $wholegene_path $intron_path
